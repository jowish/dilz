    ...
    <DiscoveryPage title={text.title} eyebrow={text.eyebrow} description={text.description} lang={lang} onLanguageChange={setLang}>
      ...
      <div className="dilz-service-grid">
        {services.map((service) => (
          <a href={service.url} target="_blank" rel="noreferrer" className={['dilz-service-card', service.featured && 'is-featured'].filter(Boolean).join(' ')} key={service.name} aria-label={`${text.discover} ${service.name}`}>
