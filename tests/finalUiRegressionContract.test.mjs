...
test('recent header fixes stay aligned and free of duplicate profile/theme controls', () => {
  ...
  assert.match(css, /@media \(max-width: 767px\)[\s\S]*\.dilz-deal-toolbar \.dilz-view-switcher__select-wrap\s*\{[^}]*width:\s*58px/s);
  assert.match(css, /@media \(max-width: 767px\)[\s\S]*\.dilz-deal-toolbar \.dilz-view-switcher__select-wrap--display\s*\{[^}]*width:\s*72px/s);
