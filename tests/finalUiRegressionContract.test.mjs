...
test('recent header fixes stay aligned and free of duplicate profile/theme controls', () => {
  ...
  // Those described a header with no notch: the header reserves
  // env(safe-area-inset-top) on top of them, so on a Dynamic Island iPhone the
  // real header is 169px tall while the bar stuck at 109px and 60px of it sat
  // behind the header — measured on main before this change. A bare pixel
  // value must never come back here.
  ...
  assert.match(css, /@media \(max-width: 767px\)\s*\{\s*:root\s*\{[^}]*--dilz-header-block:\s*109px/s);
