...
function normalizeDealInput(input = {}, { requireImage = true } = {}) {
  ...
  if (dateDebut && !/^\d{4}-\d{2}-\d{2}$/.test(dateDebut)) errors.push('Start date is invalid.');
  if (dateFin && !/^\d{4}-\d{2}-\d{2}$/.test(dateFin)) errors.push('End date is invalid.');
...
module.exports = { DEAL_CATEGORIES, LIMITS, clampLimit, normalizeDealInput, normalizeHttpUrl };
