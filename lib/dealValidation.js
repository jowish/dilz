...
function normalizeHttpUrl(value, maxLength) {
  ...
  // Bare domains (e.g. "example.com") have no scheme and fail to parse outright;
  // retry with https:// so users don't have to type a protocol themselves.
  // A string that already parses (mailto:, ftp:, javascript:, ...) keeps its
  // own scheme so it's still rejected below instead of being reinterpreted.
