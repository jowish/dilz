...
export function mergeCities(values = []) {
  const known = new Set(ISRAEL_CITIES.flatMap((item) => [item.value, item.en]));
...
export function localizedCityOptions(values = [], lang = 'en') {
  ...
  return mergeCities(values)
    .map((city) => ({ ...city, label: cityDisplayName(city.value, lang) }))
    .filter((city) => lang === 'he' ? hebrewPattern.test(city.label) : !hebrewPattern.test(city.label))
    .sort((a, b) => a.label.localeCompare(b.label, locale, { sensitivity: 'base' }));
}

export function filterCityOptions(cities = [], { search = '', lang = 'en' } = {}) {
  const locale = lang === 'he' ? 'he' : 'en';
