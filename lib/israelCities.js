...
export function cityDisplayName(city, lang = 'en') {
  ...
  return match ? match[lang === 'he' ? 'he' : 'en'] : city;
}
...
export function mergeCities(values = []) {
  ...
  const extras = values.filter(Boolean).filter((value) => !known.has(value)).map((value) => ({ value, en: value, he: value, lat: null, lon: null }));
