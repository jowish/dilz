...
export function resolveDealLayout({ requestedLayout, savedLayout } = {}) {
  if (requestedLayout === 'card' || requestedLayout === 'compact' || requestedLayout === 'list' || requestedLayout === 'spotlight') return requestedLayout;
  return ['compact', 'list', 'spotlight'].includes(savedLayout) ? savedLayout : 'card';
}
