...
export function resolveDealLayout({ requestedLayout, savedLayout } = {}) {
  if (requestedLayout === 'card' || requestedLayout === 'compact' || requestedLayout === 'list') return requestedLayout;
  return ['compact', 'list'].includes(savedLayout) ? savedLayout : 'card';
}
