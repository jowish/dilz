...
export function dealViewState(viewId, preferredSort) {
  ...
  if (viewId === 'active') return { ...state, collection: 'active' };
...
// The feed opens as rows: one deal per line, photo beside the details.
// Someone who picked another layout keeps it — only an absent choice lands here.
export const DEFAULT_DEAL_LAYOUT = 'spotlight';
...
export function resolveDealLayout({ requestedLayout, savedLayout } = {}) {
  ...
  if (['card', 'compact', 'spotlight'].includes(savedLayout)) return savedLayout;
  return DEFAULT_DEAL_LAYOUT;
}
