...
// button centres are measured). In RTL the columns are mirrored, so index 0
// sits on the right — flip the position accordingly.
export function loupeLeftFallback(center, rtl = false) {
  const effective = rtl ? (TAB_COUNT - 1 - center) : center;
  return `calc(${((effective + 0.5) / TAB_COUNT) * 100}% - ${LOUPE_WIDTH / 2}px)`;
}
...
// Map a touch x offset within the bar to a fractional tab index. In RTL the
// columns are mirrored (index 0 is on the right), so the axis is flipped.
export function touchToFraction(relX, innerWidth, rtl = false) {
  if (!innerWidth) return 0;
  const frac = clamp((relX / innerWidth) * TAB_COUNT - 0.5, 0, TAB_COUNT - 1);
  return rtl ? (TAB_COUNT - 1 - frac) : frac;
}
